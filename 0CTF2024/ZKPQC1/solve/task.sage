...
FLAG = b"0ctf{https://eprint.iacr.org/2022/475.pdf_1s_ju5t_a_proof_0f_know1edge.>.<.././././}"
FAKE_NUM = 3
...
# def hash_function(J):
#     return bytes_to_long(sha256(str(J[0]).encode()).digest()) % 2^a, \
#         bytes_to_long(sha256(str(J[1]).encode()).digest()) % 2^a
...
class ZKP:
    ...
    """
    ZKP方案参考：https://eprint.iacr.org/2022/475.pdf
    """
...
if len(s) == FAKE_NUM:
    print("You are a master of isogeny and PoK.")
    print(FLAG)